...
def MLP2_max(num_classes=40, **kwargs) -> MLP2:
    return MLP2(num_classes=num_classes, pool="max", **kwargs)

def MLP2_avg(num_classes=40, **kwargs) -> MLP2:
    return MLP2(num_classes=num_classes, pool="avg", **kwargs)


if __name__ == '__main__':
    print("===> testing pointNet with use_normals")
    data = torch.rand(10, 6, 1024)
    model = MLP2_avg(k=40, use_normals=True)
    out = model(data)
