"""
Similar to PointNet, just mlp, but deeper (layers x2).
"""
...
def MLP6_max(num_classes=40, **kwargs) -> MLP6:
    return MLP6(num_classes=num_classes, pool="max", **kwargs)

def MLP6_avg(num_classes=40, **kwargs) -> MLP6:
    return MLP6(num_classes=num_classes, pool="avg", **kwargs)


if __name__ == '__main__':
    print("===> testing pointNet with use_normals")
    data = torch.rand(10, 6, 1024)
    model = MLP6_avg(k=40, use_normals=True)
    out = model(data)
