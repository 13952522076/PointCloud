"""
Exactly like to develop5, with drop out like in develop4 for classify
"""
...
class AsymAttention(nn.Module):
    def __init__(self, dim, out_dim, heads=8, dim_head=64):
        """
        Implement Asymmetric self-attention layer k-to-1
        :param dim: input data dim
        :param heads: attention heads
        :param dim_head: dimension in each head
        """
        ...
        self.to_out = nn.Linear(inner_dim, out_dim) if project_out else nn.Identity()

    def forward(self, x):
        """
        Using Asymmetric self-attention gather all features to fist one
        :input x: [b batch, k points, n nerigbhors, d dimension]
        :return: [b batch, k points, 1 nerigbhors, d dimension]
        """
        b, k, n, _, h = *x.shape, self.heads
        sample = (x[:, :, 0, :]).clone().unsqueeze(dim=-2)
        k = self.to_k(sample)
        q = self.to_q(x)
        v= self.to_v(x-sample)
        q, k, v = map(lambda t: rearrange(t, 'b k n (h d) -> b k h n d', h=h), [q, k, v])

        dots = einsum('b k h i d, b k h j d -> b k h i j', k, q) * self.scale
        attn = self.attend(dots)
        out = einsum('b k h i j, b k h j d -> b k h i d', attn, v)
        out = rearrange(out, 'b k h n d -> b k n (h d)')
        out = self.to_out(out)
...
class TransformerDown(nn.Module):
    def __init__(self, in_dim, out_dim, **kwargs):
        ...
        self.map_sample = nn.Linear(in_dim, out_dim,bias=False)
        self.map_diff = nn.Linear(in_dim, out_dim, bias=False)

    def forward(self, x):
        x = self.norm1(x)
        sample = (x[:, :, 0, :])   # [b, sample, 1,d]
        neighbors = x[:, :, 1:, :]  # [b, sample, neighbor-1,d] ,exclude itself
        difference = neighbors - sample.unsqueeze(dim=-2)
        att = torch.softmax(difference.mean(dim=-1,keepdim=True),dim=-2)
        offset = (self.map_diff(difference)*att).sum(dim=-2)
...
class Develop5(nn.Module):
    def __init__(self, num_classes=40, use_normals=True, points=512,
        ...
        self.classify = nn.Sequential(
            nn.Linear(embed_channel * factor * expansion, (embed_channel * factor * expansion) // 4),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear((embed_channel * factor * expansion) // 4, num_classes)
        )
...
def develop5Amax(num_classes=40, **kwargs: Any) -> Develop5:
    return Develop5(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", **kwargs)


if __name__ == '__main__':
    # print("===> testing asymattention module ...")
    # data = torch.rand(32, 64, 6, 128)  # [b batch, p points, k nerigbhors, d dimension]
    # model = AsymAttention(128, 256)
    # out = model(data)
    # print(out.shape)
    print("===> testing TransformerDown ...")
    data = torch.rand(32, 64, 6, 128)  # [b batch, p points, k nerigbhors, d dimension]
    model = TransformerDown(128, 256)
    out = model(data)
    print(out.shape)
    ...
    print("===> testing develop5Amax ...")
    pointsformer = develop5Amax()
