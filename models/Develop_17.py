...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, att_type="subtraction"):
        """
    ...
    def forward(self, x):
        ...
        if self.att_type =="subtraction":
            q,k,v = qkv
            subtraction = q.unsqueeze(dim=-2)-k.unsqueeze(dim=-3)  # [b, p, heads, k-nerighbors, k-nerighbors head-dim]
            attn = subtraction.softmax(dim=-2)
            out = (attn*(v.unsqueeze(dim=-3))).sum(dim=-2,keepdim=False)
        else:
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, att_type="subtraction", **kwargs):
        """
        ...
        self.attention = Attention(dim=dim, heads=heads, dim_head=dim_head, att_type=att_type)
...
class Develop17(nn.Module):
    def __init__(self, num_classes=40, use_normals=True, points=512,
                 blocks=[1, 2, 1, 1], embed_channel=32, k_neighbors=[16, 16, 16, 16],
                 heads=8, dim_head=16, expansion=2, reducer=4, pool="avg", att_type="subtraction", **kwargs):
        super(Develop17, self).__init__()
        ...
        for stage, block_num in enumerate(blocks):
            ...
            for _ in range(block_num):
                transformer_blocks.append(
                    TransformerBlock(dim=embed_channel * factor, heads=heads * factor_h,
                                     dim_head=dim_head * factor_d, att_type=att_type)
                )
...
def develop17Adot(num_classes=40, **kwargs: Any) -> Develop17:
    return Develop17(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", att_type="dotproduct", **kwargs)

def develop17Asub(num_classes=40, **kwargs: Any) -> Develop17:
    return Develop17(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", att_type="subtraction", **kwargs)
...
if __name__ == '__main__':
    ...
    print("===> testing develop17Adot ...")
    pointsformer = develop17Adot()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing develop17Asub ...")
    pointsformer = develop17Asub()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)
