...
class ChannelAttention(nn.Module):
    def __init__(self, dim, reduction=16):
        ...
        self.ca = nn.Sequential(
            nn.Linear(dim, max(8, dim//reduction)),
            nn.ReLU(inplace=True),
            nn.Linear(max(8, dim//reduction),dim),
            nn.Sigmoid()
        )
...
def develop14A(num_classes=40, **kwargs: Any) -> Develop14:
    return Develop14(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, **kwargs)

def develop14Amax(num_classes=40, **kwargs: Any) -> Develop14:
    return Develop14(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", **kwargs)

def develop14Bmax(num_classes=40, **kwargs: Any) -> Develop14:
    return Develop14(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, embed_channel=64, pool="max",  **kwargs)
...
if __name__ == '__main__':
    ...
    print("===> testing develop14A ...")
    pointsformer = develop14A()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing develop14Amax ...")
    pointsformer = develop14Amax()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)
