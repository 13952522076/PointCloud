...
class MLPnoBN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(self.dropout(self.fc2(x)), inplace=True)
        x = self.fc3(x)
        return {
            "logits":x
        }
...
def MLPnoBN_max(num_classes=40, **kwargs) -> MLPnoBN:
    return MLPnoBN(num_classes=num_classes, pool="max", **kwargs)

def MLPnoBN_avg(num_classes=40, **kwargs) -> MLPnoBN:
    return MLPnoBN(num_classes=num_classes, pool="avg", **kwargs)


if __name__ == '__main__':
    print("===> testing pointNet with use_normals")
    data = torch.rand(10, 6, 1024)
    model = MLPnoBN_max(k=40, use_normals=True)
    out = model(data)
