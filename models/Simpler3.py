...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        :return: [b batch, p points, k nerigbhors, d dimension]
        """
        att = F.relu(self.attention(self.norm1(x)), inplace=True)
        att = att + x
...
class Simpler3(nn.Module):
    def __init__(self, num_classes=40, use_normals=True, points=512,
        ...
        self.classify = nn.Sequential(
            nn.Linear(embed_channel * factor * expansion, (embed_channel * factor)//2),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear((embed_channel * factor)//2, num_classes)
        )
...
def Simpler3A(num_classes=40, **kwargs: Any) -> Simpler3:
    return Simpler3(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, **kwargs)

def Simpler3B(num_classes=40, **kwargs: Any) -> Simpler3:
    return Simpler3(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, dim_head=32, **kwargs)

def Simpler3C(num_classes=40, **kwargs: Any) -> Simpler3:
    return Simpler3(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, heads=16, **kwargs)

def Simpler3D(num_classes=40, **kwargs: Any) -> Simpler3:
    return Simpler3(num_classes=num_classes, blocks=[2, 2, 2, 2], reducer=4, **kwargs)

def Simpler3E(num_classes=40, **kwargs: Any) -> Simpler3:
    return Simpler3(num_classes=num_classes, blocks=[3, 4, 6, 3], reducer=4, **kwargs)
...
if __name__ == '__main__':
    ...
    print("===> testing Simpler3A ...")
    pointsformer = Simpler3A()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing Simpler3B ...")
    pointsformer = Simpler3B()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing Simpler3C ...")
    pointsformer = Simpler3C()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing Simpler3D ...")
    pointsformer = Simpler3D()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing Simpler3E ...")
    pointsformer = Simpler3E()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)
