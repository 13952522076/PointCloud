...
class TransformerDown(nn.Module):
    def __init__(self, in_dim, out_dim, heads=8, dim_head=64, **kwargs):

        super(TransformerDown, self).__init__()
        self.norm1 = nn.LayerNorm(in_dim)
        self.norm2 = nn.LayerNorm(out_dim)
        self.attention = AsymAttention(dim=in_dim, out_dim=out_dim, heads=heads, dim_head=dim_head)
    ...
    def forward(self, x):
        x = self.norm1(x)
        sample = (x[:, :, 0, :]).clone().unsqueeze(dim=-2)
        offset = self.attention(x)
        sample = offset + self.map(sample)
        out = self.ffn(self.norm2(sample))
        out = out + sample
        return out
...
class Develop2(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.stages):
            ...
            coords = coords[:,:,0,:]
            out = self.transformer_downs[i](features).squeeze(dim=-2)
...
def develop2Amax(num_classes=40, **kwargs: Any) -> Develop2:
    return Develop2(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", **kwargs)
...
if __name__ == '__main__':
    ...
    print("===> testing develop2Amax ...")
    pointsformer = develop2Amax()
