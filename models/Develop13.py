"""
only global attention, without local
a develop13 template, copyed from simple2.py
"""
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, **kwargs):
        ...
        self.g_norm1 = nn.LayerNorm(dim)
        self.g_norm2 = nn.LayerNorm(dim)
        self.g_attention = Attention(dim=dim, heads=heads, dim_head=dim_head)
        self.g_ffn = FeedForward(dim=dim, hidden_dim=dim)  # modify hidden_dim accordingly, e.g, 2*dim or dim/2.
    ...
    def forward(self, x):
        ...
        g = x[:,:,0,:].clone()
        g = g.unsqueeze(dim=1)
        g_att = self.g_attention(self.g_norm1(g))
        g_att = g_att + g
        g_out = self.g_ffn(self.g_norm2(g_att))
        g_out = g_out + g_att
        g_out = g_out.permute(0, 2, 1, 3)
...
def develop13A(num_classes=40, **kwargs: Any) -> Develop13:
    return Develop13(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, **kwargs)

def develop13Amax(num_classes=40, **kwargs: Any) -> Develop13:
    return Develop13(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", **kwargs)
...
if __name__ == '__main__':
    ...
    print("===> testing develop13A ...")
    pointsformer = develop13A()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)

    print("===> testing develop13Amax ...")
    pointsformer = develop13Amax()
    data = torch.rand(2, 6, 1024)
    out = pointsformer(data)
    print(out["logits"].shape)
